...
async def forward(dut):
    ...
    for i in range(255):
        ...
        # check output
        assert int(dut.data_out.value) == sbox[i], f"Data mismatch on cycle {i}"
...
async def inverse(dut):
    ...
    for i in range(255):
        ...
        # check output
        assert int(dut.data_out.value) == sboxInv[i], f"Data mismatch on cycle {i}"

    # end
    dut.data_in.value = 0
    await Timer(100, units="ns")

def test_stv_aes_sbox():
     
    hdl_toplevel = "stv_aes_sbox"
